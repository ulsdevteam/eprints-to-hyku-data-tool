...
OUTPUT_DIRECTORY = "/mounts/data/hyku/output/generic"
...
def parse_object(json_object):
	...
	if 'rights' not in json_object.keys() or not json_object['rights']:
		new_object['rights'] = 'http://rightsstatements.org/vocab/CNE/1.0/'
	...
	# resource type field
	# required
	if 'resource_type' in json_object.keys():
		new_object['resource_type'] = json_object['resource_type']
	if 'resource_type' not in json_object.keys() or not json_object['resource_type']:
		new_object['resource_type'] = 'Other'
		log_activity_to_file("Object \""+new_object['source_identifier']+"\" is missing required field: resource_type", LOGFILE_DEFAULT_ERROR)
		log_activity_to_file("Object \""+new_object['source_identifier']+"\" is missing required field: resource_type", LOGFILE_DEFAULT_DETAILS)
		with_errors = True
